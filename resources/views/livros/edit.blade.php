            ...
            <div class="form-control">
                ...
                <div class="flex items-center gap-3">
                    <input type="number" id="estoque" name="estoque" min="0"
                        value="{{ old('estoque', $livro->estoque ?? 0) }}"
                        placeholder="Quantidade disponível"
                        class="input input-bordered flex-1" />
                    <div class="flex flex-col text-sm">
                        <span class="text-gray-500">Atual:
                            <span class="font-semibold {{ ($livro->estoque ?? 0) > 0 ? 'text-success' : 'text-error' }}">
                                {{ $livro->estoque ?? 0 }} unidade(s)
                            </span>
                        </span>
                        @if(($livro->estoque ?? 0) == 0)
                        <span class="text-error text-xs">⚠️ Livro esgotado</span>
                        @endif
                    </div>
                </div>
            </div>
